...
function get_ricetta_share($ricetta_id, $code) {
	$ricetta_id = (int)$ricetta_id;
	$code = sanitize($code);

	if( mysql_result(mysql_query("SELECT COUNT('ricetta_id') from ricette WHERE ricetta_id = '$ricetta_id'"), 0)!= 0) {
		
		$query = mysql_query("SELECT * FROM ricette WHERE ricetta_id = '$ricetta_id' ");
		
		$ricetta = mysql_fetch_array($query);

		if(substr(md5($ricetta["username"].$ricetta["ricetta_id"]), 0, 5) === $code) {
			return $ricetta;
		}else {
			return "codice_non_valido";
		}
		
	} else {
		return "nessuna_ricetta_da_id";
	}
}
